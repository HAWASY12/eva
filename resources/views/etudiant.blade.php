...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container mt-6" >
        ...
        <div style="background:#B0C4DE; border-radius:5px;padding:5px">
                <h1 style="background:#B0C4DE"; class="text-center mt-0"><marquee> Liste des Etudiants</marquee></h1>
            </div>
            <br>
            <br>
            <br>
            ...
            <span hidden>{{$somme=0}}</span>
            @foreach($etudiants as $r)
            @foreach($r->matieres as $m)

            <span hidden>{{$somme= $m->pivot->note + $m->pivot->examen}}</span>
            @endforeach
            @endforeach
            <h2>la moyenne generale est: {{$somme / $etudiants->count()}}</h2>
</div>
